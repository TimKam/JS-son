...
const paranoid = new RemoteAgent('paranoid', beliefs, function () {
  if (global.paranoidActions) {
    return global.paranoidActions;
  }
  return {};
});

const plansClaustrophobe = 1;
...
function render (state) {
  ...
  Object.keys(state.actions).forEach(function (agentId) {
    const message = `${agentId}: ${state.actions[agentId]}`; // ${global.paranoidActions ? JSON.stringify(global.paranoidActions) : ''}
    g.drawString(message, (128 - g.stringWidth(message)) / 2, 20 + agentId * 10);
    Bluetooth.println(`${message} [${JSON.stringify(state)}]`);
  });
  g.flip(true);
}
