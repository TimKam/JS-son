...
const setupNetwork = (learningRate = 1e-3, activationType = 'relu', height = 20, width = 20) => {
  const optimizer = tensorflow.train.adam(learningRate)

  const activation = activationType

  const createNetwork = (numActions) => {
  ...
  const trainOnReplayBatch = (gamma, optimizer, memory, policy) => {
    const lossFunction = () => tensorflow.tidy(() => {
      const stateTensor = getStateTensor(
        memory.map(example => example[0]), height, width)
      ...
      const nextStateTensor = getStateTensor(
        memory.map(example => example[4]), height, width)
  ...
  return { createNetwork, trainOnReplayBatch, optimizer, determineAction }
}
